...
export async function POST(request) {
  try {
    ...
    if (!existing || !existing.checkIn) {
      return NextResponse.json({ error: 'Please check in first before checking out.' }, { status: 400 });
    }
    ...
    if (employee.assignedLocation !== 'either' && location.label !== employee.assignedLocation) {
      ...
      return NextResponse.json(
        { error: `You're not near ${zoneName} — you must be within range of your assigned location to check out.` },
        { status: 400 }
      );
    }

    existing.checkOut = { time: now, lat, lng, photo, faceMatchDistance, faceMatchStatus, locationLabel: location.label, distanceMeters: location.distanceMeters };
    existing.updatedAt = now;
    await existing.save();

    return NextResponse.json({ ok: true, faceMatchStatus, locationLabel: location.label });
  } catch (err) {
